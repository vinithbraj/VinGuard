    ...
    public:
        ...
        static NTSTATUS unload_filter(FLT_FILTER_UNLOAD_FLAGS Flags)
        {
            ...
            if (filter_callback::g_filter_comm) {
                filter_callback::g_filter_comm->shutdown();
                VinGuard::memory::free_kernel_object(filter_callback::g_filter_comm);
            }
        ...
        static NTSTATUS load_filter(PDRIVER_OBJECT DriverObject)
        {
            di("load_filter - enter");
             ...
             filter_callback::g_filter_comm = VinGuard::memory::allocate_simple_pod_kernel_object<VinGuard::kernel_user_com>();
             if (!filter_callback::g_filter_comm)
             {
             ...
             status = filter_callback::g_filter_comm->initialize(VinGuard::filter_registration::s_filter_handle);
             if (!NT_SUCCESS(status))
             {
